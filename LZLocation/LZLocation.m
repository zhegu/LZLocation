...
+ (CLLocationCoordinate2D)locationMarsFromEarth:(CLLocationCoordinate2D)coordinate
{
    double lat = 0.0;
    double lng = 0.0;
    transform_earth_from_mars(coordinate.latitude, coordinate.longitude, &lat, &lng);
    
    return CLLocationCoordinate2DMake(lat+coordinate.latitude, lng+coordinate.longitude);
}
...
+ (CLLocationCoordinate2D)locationEarthFromMars:(CLLocationCoordinate2D)coordinate
{
    ...
    double lng = 0.0;
    transform_earth_from_mars(coordinate.latitude, coordinate.longitude, &lat, &lng);
    return CLLocationCoordinate2DMake(coordinate.latitude-lat, coordinate.longitude-lng);
...
+ (CLLocationCoordinate2D)locationEarthFromBaidu:(CLLocationCoordinate2D)coordinate
{
    double lat = 0.0;
    double lng = 0.0;
    CLLocationCoordinate2D Mars = [LZLocation locationMarsFromBaidu:coordinate];
